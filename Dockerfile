...
RUN [ -e /usr/bin/upx ] && upx /go/bin/sponsorship-notify || echo
FROM scratch
COPY --link --from=build-dev /go/bin/sponsorship-notify /go/bin/sponsorship-notify
COPY --from=build-dev /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/
ENTRYPOINT ["/go/bin/sponsorship-notify"]
